...
; (5) typecheck: TFAE TypeEnv -> Type
; (6) purpose: to check that the given 'tfae' is the appropriate TYPE.
...
; (7) eval : TFAE -> TFAE-value
; (8) purpose: to do typecheck and interp in one queue(if type is invalid just show error message and skip interpreting)
...
(test/exn (eval (app (id 'x) (num 5))) "type-error: typecheck") ; (9) Why is this type-error? (first arguments' type of 'app' branch should be 'arrowT' but it's not)
(test/exn (eval (app (fun 'x (numTE) (id 'x)) (fun 'x (numTE) (id 'x)))) "type-error: typecheck"); (10) Why is this type-error?:(Type of second 'app' branch arguments should be numT(same type with function's parameter))